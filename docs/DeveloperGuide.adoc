...
image::SDforDeletePlaceEventHandling.png[width="800"]
...
For version 1.1 and 1.2, the theme color of the GUI is chosen randomly between Black and White in link:{repoURL}/src/main/resources/view/MainWindow.fxml[`MainWindow.fxml`], by choosing link:{repoURL}/src/main/resources/view/MainWindow_Black.fxml[`MainWindow_Black.fxml`] or link:{repoURL}/src/main/resources/view/MainWindow_White.fxml[`MainWindow_White.fxml`]
...
.  The command execution can affect the `Model` (e.g. adding a place) and/or raise events.
.  The result of the command execution is encapsulated as a `CommandResult` object which is passed back to the `Ui`.

Given below is the Sequence Diagram for interactions within the `Logic` component for the `execute("delete 1")` API call.

image::DeletePlaceSdForLogic.png[width="800"]
...
Both BM and CBM extends from `UndoableCommand` as shown in **Figure 2.3.2** , which allows Undo/Redo by the user.
Also, BM is an index based command like (`delete` or `edit`) +
...
=== Displaying basic tourism components
...
For command `weather`, link:{repoURL}/src/main/java/seedu/address/logic/commands/WeatherCommand.java[`WeatherCommand`] is called then it calls the function `loadpage` from `browserPanel` on link:{repoURL}/src/main/java/seedu/address/ui/MainWindow.java[`MainWindow.java`], to open an URL of a weather forecast website on the `BrowserPanel`

Similar to `weather`, PSI values of can be access by typing `psi` in the `CommandBox`.

For clicking the 'Weather' tab in the `MenuBar`, the `handleWeather` function is called, by FXML, on link:{repoURL}/src/main/java/seedu/address/ui/MainWindow.java[`MainWindow.java`].
...
Where `browserPanel` is the object link:{repoURL}/src/main/java/seedu/address/ui/BrowserPanel.java[`BrowserPanel`] on link:{repoURL}/src/main/java/seedu/address/ui/MainWindow.java[`MainWindow.java`]
...
image::UiComponentwithMrtClassDiagram.png[width="800"]
_Figure 3.6.1 : Structure of the UI Component with MRTWindow_
...
image::MRTSequenceDiagrams.jpg[width="600"]
_Figure 3.6.2 : Structure of the UI Component with MRTWindow_
...
image::MrtMapButtonUI.png[width="600"]
_Figure 3.6.3 : UI button added to the `MainWindow`.
...
image::SortCommandSequenceDiagram.png[width="600"]

_Figure 3.7.1 : Sequence diagram of `sort` command.
...
// tag::goto[]
The command takes the INDEX from the argument and get the name of the `place` in the list, in link:{repoURL}/src/main/java/seedu/address/logic/commands/BrowserPanel.java[`GotoCommand.java`], and pass it to the method `loadUrl` in `browserPanel' in link:{repoURL}/src/main/java/seedu/address/ui/MainWindow.java[`MainWindow`].

The method `loadUrl` will pass the Google Map url to link:{repoURL}/src/main/java/seedu/address/ui/BrowserPanel.java[`BrowserPanel`] to display the location in `BrowserPanel`.
...
_Figure 3.9A - Class Diagram of ExportCommand_

From _Figure 3.9A_, the ExportCommand depends on `java.io` to carry out the file IO operations to create the CSV file.
 ...
 _Figure 3.9B - Sequence Diagram of ExportCommand_

 Explanation of _Figure 3.9B_:
 ...
 * Able to easily construct the `String` due to the mutability of `StringBuilder`.

 *Cons:*

 * Dependency on the `StringBuilder` class.

 *Alternative 2:* Using `String` only.

 *Pros:*

 * Reduced dependency on additional classes.

 *Cons:*

 * Constant re-assignment of resultant `String` is required as it is not mutable.
 ...
 * Code is executed at a faster rate.

 *Cons:*

 * Coupling is increased with `ReadOnlyPlace`.

 * Code is not adaptable should new fields be added to `ReadOnlyPlace`.

 *Alternative 2:* Generate place data according to the fields present in `ReadOnlyPlace`.

 *Pros:*

 * Code is adaptable should new fields be added to `ReadOnlyPlace`.
...
The implementation of the method is credited to link:https://github.com/AceCentury[Ryan Teo] and +
link:https://github.com/nus-cs2103-AY1718S1/forum/issues/158[this issue].
//end::exportCommand[]
...
. Add a shorthand equivalent alias for each of the individual commands. For example, besides typing `clear`, the user can also type `c` to remove all places in the list.
+
****
* Hints
...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/address/model/place/Place.java[`Place`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] allows you to update a place, and link:{repoURL}/src/main/java/seedu/address/model/place/Place.java[`Place`] allows you to update the tags.
* Solution
** Add the implementation of `deleteTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/model/ModelManager.java[`ModelManager`]. Loop through each place, and remove the `tag` from each place.
...
. Use different colors for different tags inside place cards. For example, `friends` tags can be all in grey, and `colleagues` tags can be all in red.
+
**Before**
+
...
****
* Hints
** The tag labels are created inside link:{repoURL}/src/main/java/seedu/address/ui/PlaceCard.java[`PlaceCard#initTags(ReadOnlyPlace)`] (`new Label(tag.tagName)`). https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Label.html[JavaFX's `Label` class] allows you to modify the style of each Label, such as changing its color.
...
* Solution
** Modify the constructor of link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to take in the number of places when the application just started.
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the number of places whenever there are new changes to the addressbook.
...
Let's add a placeholder on all our link:{repoURL}/src/main/java/seedu/address/ui/PlaceCard.java[`PlaceCard`] s to display a remark for each place later.

**Main:**

. Add a `Label` with any random text inside link:{repoURL}/src/main/resources/view/PlaceListCard.fxml[`PlaceListCard.fxml`].
. Add FXML annotation in link:{repoURL}/src/main/java/seedu/address/ui/PlaceCard.java[`PlaceCard`] to tie the variable to the actual label.

**Tests:**

. Modify link:{repoURL}/src/test/java/guitests/guihandles/PlaceCardHandle.java[`PlaceCardHandle`] so that future tests can read the contents of the remark label.
[disceret]
===== [Step 4] Model: Add `Remark` class
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/address/model/place/ReadOnlyPlace.java[`ReadOnlyPlace`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.

**Main:**

. Add `Remark` to model component (you can copy from link:{repoURL}/src/main/java/seedu/address/model/place/Address.java[`Address`], remove the regex and change the names accordingly).
...
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/address/model/place/ReadOnlyPlace.java[`ReadOnlyPlace`].

**Main:**

. Add three methods `setRemark(Remark)`, `getRemark()` and `remarkProperty()`. Be sure to implement these newly created methods in link:{repoURL}/src/main/java/seedu/address/model/place/ReadOnlyPlace.java[`Place`], which implements the link:{repoURL}/src/main/java/seedu/address/model/place/ReadOnlyPlace.java[`ReadOnlyPlace`] interface.
. You may assume that the user will not be able to use the `add` and `edit` commands to modify the remarks field (i.e. the place will be created without a remark).
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `addressBook.xml` so that the application will load the sample data when you launch it.)
[disceret]
===== [Step 6] Storage: Add `Remark` field to `XmlAdaptedPlace` class
We now have `Remark` s for `Place` s, but they will be gone when we exit the application. Let's modify link:{repoURL}/src/main/java/seedu/address/storage/XmlAdaptedPlace.java[`XmlAdaptedPlace`] to include a `Remark` field so that it will be saved.

**Main:**

. Add a new Xml field for `Remark`.
. Be sure to modify the logic of the constructor and `toModelType()`, which handles the conversion to/from  link:{repoURL}/src/main/java/seedu/address/model/place/ReadOnlyPlace.java[`ReadOnlyPlace`].
...
Our remark label in link:{repoURL}/src/main/java/seedu/address/ui/PlaceCard.java[`PlaceCard`] is still a placeholder. Let's bring it to life by binding it with the actual `remark` field.

**Main:**

. Modify link:{repoURL}/src/main/java/seedu/address/ui/PlaceCard.java[`PlaceCard#bindListeners()`] to add the binding for `remark`.

**Tests:**

. Modify link:{repoURL}/src/test/java/seedu/address/ui/testutil/GuiTestAssert.java[`GuiTestAssert#assertCardDisplaysPlace(...)`] so that it will compare the remark label.
. In link:{repoURL}/src/test/java/seedu/address/ui/PlaceCardTest.java[`PlaceCardTest`], call `placeWithTags.setRemark(ALICE.getRemark())` to test that changes in the link:{repoURL}/src/main/java/seedu/address/model/place/ReadOnlyPlace.java[`Place`] 's remark correctly updates the corresponding link:{repoURL}/src/main/java/seedu/address/ui/PlaceCard.java[`PlaceCard`].
[disceret]
...
. Replace the logic in `RemarkCommand#execute()` (that currently just throws an `Exception`), with the actual logic to modify the remarks of a place.
...
[discrete]
=== Problem statement
Due to a large number of attractions and different websites, we want a platform that is intuitive to find attractions.
The user should not need to view multiple sources to decide where to go. Relevant information to aid their decision making
...
|`* * *` |new user |view the user guide easily |learn more about the functions of addressbook as and when I need

|`* * *` |user |see all places at a glance |determine where to go

|`* * *` |user |use shorter versions of a commandn |use it faster

|`* * *` |user |bookmark my favorite places |easily reference them later

|`* * *` |user |remove my bookmark |update my latest preferences

|`* * *` |user |find places with a certain tag |narrow down my search result

|`* * *` |user |see all the places  |choose where I want to go

|`* * *` |user |see the history of the command that I entered |keep track of the changes that I made

|`* * *` |user |undo and redo commands |revert back any mistakes that I made

|`* * *` |user |add places to address book |easily view the summary of each place

|`* * *` |user |be able to search all places by name |I am able to find a specific place

|`* * *` |user |edit the existing places |fix any errors or update information

|`* * *` |user |delete a place  |not have it  appear anymore in my search results

|`* * *` |user |exit the program  |stop it from consuming background resources

|`* *` |user |see the basic laws of Singapore  |avoid getting into trouble

|`* *` |user |go to a attraction's homepage  |see the attraction's profile and description

|`* *` |user |search for places from names, phone numbers, emails and tags  |search for it just by knowing 1

|`* *` |user |search the place address via google map  |see the location details

|`* *` |user |view the weather  |plan my day around Singapore

|`* *` |user |not see duplicate entries  |avoid removing redundant entries

|`* *` |user |see the direction from place A to place B  |do navigation

|`* *` |user |open up the public transport map  |plan my journey

|`* *` |user |see Singapore's emergency hotlines  |know who to call during an emergency

|`* *` |user |view the PSI of the day  |plan my day around Singapore

|`*` |user |easily customize the color scheme |make it to my liking

|`*` |user |set notification ringtone to the certain places |I can differentiate what type of place is it from

|`*` |user | add the places founded date to the calendar |I can make keep track of it's anniversary.

|`*` |user |hide certain places |ignore places that I visited

|`*` |user |see the clock |know what the time it is

|`*` |user |the place in charge of the places |contact when necessary

|`*` |user |be notified of promotions ongoing at different places |visit the places during the promotional period

|`*` |user |see the events happening in Singapore |have a rough idea of where I'm going to go

|`*` |user |print my address book |bring it around without logging on to my laptop

|`*` |user |see the exchange rate for Singapore |budget my expenses

|`*` |user |book tickets online for the attractions |avoid needing to queue at the ticketing counter
...
* Steps 2a1-2a2 are repeated until user is satisfied with result
Use case continue from step 3.
...
** Steps 2a1-2a2 are repeated until the keyword entered is correct
Use case continue from step 3.
